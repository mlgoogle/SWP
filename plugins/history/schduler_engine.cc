...
void HistoryManager::SendHistoryTrades(const int socket, const int64 uid,
  ...
  while (trades_list.size() > 0) {
    ...
    net_reply::TradesPosition* net_trades_position =
      ...
      if (net_trades_positions.Size() % base_num == 0
        ...
        MAKE_HEAD(packet_control, S_HISTORY_TRADES, HISTORY_TYPE, 0, 0,
                  0);
  ...
  if (net_trades_positions.Size() > 0) {
    struct PacketControl packet_control;
    MAKE_HEAD(packet_control, S_HISTORY_TRADES, HISTORY_TYPE, 0, 0, 0);
    packet_control.body_ = net_trades_positions.get();
    send_message(socket, &packet_control);
  }
}
...
void HistoryManager::GetHistoryTradesNoLock(
  ...
  for(TRADES_MAP::iterator it = trades_map.begin(); it != trades_map.end(); it++) {
